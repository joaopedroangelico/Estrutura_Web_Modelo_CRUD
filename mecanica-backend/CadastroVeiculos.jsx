...
const VEICULO_INICIAL = { placa: '', modelo: '', cor: '', descricao: '', status: 'iniciado', valor: '' }
...
export default function CadastroVeiculos() {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        setErros([data.erro || 'Erro ao salvar ordem de servico.'])
        setSalvando(false)
        return
      }
  ...
  return (
    <div style={pageStyle}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
        <h1 style={tituloPaginaStyle}>Nova Ordem de Servico</h1>
        <button style={btnCancelarStyle} onClick={() => navigate('/dashboard')}>Voltar ao Dashboard</button>
      </div>
      ...
      <div style={secaoStyle('#3b82f6')}>
        ...
        <div style={gridStyle}>
          ...
          <div style={fieldFullStyle}>
            ...
            <textarea style={textareaStyle} placeholder="Descreva o servico a ser realizado..."
      ...
      <div style={acoesStyle}>
        ...
        <button style={{ ...btnSalvarStyle, opacity: salvando ? 0.7 : 1 }} onClick={handleSalvar} disabled={salvando}>
          {salvando ? 'Salvando...' : 'Salvar Ordem de Servico'}
        </button>
      </div>
    </div>
  )
